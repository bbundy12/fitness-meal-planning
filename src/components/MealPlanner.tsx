...
export function MealPlanner({ onNavigateToRecipeSelector }: MealPlannerProps) {
  ...
  const availableRecipes: Recipe[] = [
    { id: "r1", title: "Grilled Chicken with Rice", servings: 2, calories: 520, protein: 45, carbs: 52, fat: 12 },
    { id: "r2", title: "Salmon and Sweet Potato", servings: 2, calories: 480, protein: 38, carbs: 45, fat: 18 },
    { id: "r3", title: "Greek Yogurt Parfait", servings: 2, calories: 280, protein: 24, carbs: 36, fat: 6 },
    { id: "r4", title: "Protein Smoothie", servings: 2, calories: 320, protein: 30, carbs: 28, fat: 10 },
    { id: "r5", title: "Turkey Sandwich", servings: 2, calories: 420, protein: 32, carbs: 48, fat: 12 },
    { id: "r6", title: "Egg White Omelet", servings: 2, calories: 250, protein: 28, carbs: 12, fat: 8 }
  ];

  const [draggedRecipe, setDraggedRecipe] = useState<Recipe | null>(null);
  const [repeatDaily, setRepeatDaily] = useState(false);
  const [weekPlan, setWeekPlan] = useState<{ [key: string]: DayPlan }>(
    DAYS.reduce((acc, day) => ({
      ...acc,
      [day]: {
  ...
  const copyMondayToAllDays = () => {
    ...
    DAYS.forEach(day => {
      if (day !== "Monday") {
        updatedPlan[day] = {
          breakfast: { ...updatedPlan[day].breakfast, recipe: mondayPlan.breakfast.recipe },
          snack1: { ...updatedPlan[day].snack1, recipe: mondayPlan.snack1.recipe },
          lunch: { ...updatedPlan[day].lunch, recipe: mondayPlan.lunch.recipe },
          snack2: { ...updatedPlan[day].snack2, recipe: mondayPlan.snack2.recipe },
          dinner: { ...updatedPlan[day].dinner, recipe: mondayPlan.dinner.recipe },
          snack3: { ...updatedPlan[day].snack3, recipe: mondayPlan.snack3.recipe }
        };
      }
    });
    setWeekPlan(updatedPlan);
  ...
  const resetWeekPlan = () => {
    if (window.confirm('Are you sure you want to reset the entire week? This will clear all meals.')) {
      setWeekPlan(
        DAYS.reduce((acc, day) => ({
          ...acc,
          [day]: {
  ...
  const getDayTotals = (dayPlan: DayPlan) => {
    const meals = [dayPlan.breakfast, dayPlan.snack1, dayPlan.lunch, dayPlan.snack2, dayPlan.dinner, dayPlan.snack3];
          ...
          <div className="flex flex-wrap gap-2">
            {availableRecipes.slice(0, 6).map((recipe) => (
              <div
                ...
                className="px-3 py-2 bg-rose-100 text-rose-800 rounded-lg cursor-move hover:bg-rose-200 transition-colors"
              >
            ...
            <Card key={day}>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline">
                      {dayTotals.calories} cal | P: {dayTotals.protein}g | C: {dayTotals.carbs}g | F: {dayTotals.fat}g
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-3 bg-slate-50 rounded-lg">
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-slate-900">{dayTotals.calories}/{dailyGoals.calories}</span>
                      </div>
                      <Progress value={getProgress(dayTotals.calories, dailyGoals.calories)} className="h-2" />
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-slate-600">Protein</span>
                        <span className="text-slate-900">{dayTotals.protein}g/{dailyGoals.protein}g</span>
                      </div>
                      <Progress value={getProgress(dayTotals.protein, dailyGoals.protein)} className="h-2" />
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-slate-600">Carbs</span>
                        <span className="text-slate-900">{dayTotals.carbs}g/{dailyGoals.carbs}g</span>
                      </div>
                      <Progress value={getProgress(dayTotals.carbs, dailyGoals.carbs)} className="h-2" />
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-slate-600">Fat</span>
                        <span className="text-slate-900">{dayTotals.fat}g/{dailyGoals.fat}g</span>
                      </div>
                      <Progress value={getProgress(dayTotals.fat, dailyGoals.fat)} className="h-2" />
                    </div>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  {(['breakfast', 'snack1', 'lunch', 'snack2', 'dinner', 'snack3'] as const).map((mealType) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => !slot.recipe && onNavigateToRecipeSelector?.(day, mealType)}
                        className={`p-4 rounded-lg border-2 border-dashed min-h-[120px] transition-colors ${
                          slot.recipe 
                            ? 'border-rose-300 bg-rose-50' 
                            : 'border-slate-200 bg-slate-50 hover:border-rose-300 hover:bg-rose-50 cursor-pointer'
                        }`}
                      >
