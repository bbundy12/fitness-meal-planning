...
interface InBodyScanLogProps {
  currentUser: string;
}

export function InBodyScanLog({ currentUser }: InBodyScanLogProps) {
  ...
  const getTrend = (current: number, previous: number) => {
    ...
    if (current > previous) return <TrendingUp className="h-4 w-4 text-rose-600" />;
  ...
  const calculateProgress = (current: ScanEntry, previous: ScanEntry): ProgressDeltas => {
    ...
    return {
      ...
      leanMassChange: Math.round((currentMetrics.leanMassLbs - previousMetrics.leanMassLbs) * 10) / 10,
      ...
      {latestProgress && scans.length >= 2 && (
        <Card className="bg-rose-50 border-rose-200">
          <CardHeader>
            ...
            <p className="text-slate-500">Changes from {new Date(scans[1].date).toLocaleDateString()} to {new Date(scans[0].date).toLocaleDateString()}</p>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <div className="text-slate-900">
                    {latestProgress.weightChange > 0 ? '+' : ''}{latestProgress.weightChange} lbs
                  </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <div className="text-slate-900">
                    {latestProgress.fatMassChange > 0 ? '+' : ''}{latestProgress.fatMassChange} lbs
                  </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <div className="text-slate-900">
                    {latestProgress.leanMassChange > 0 ? '+' : ''}{latestProgress.leanMassChange} lbs
                  </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <div className="text-slate-900">
                    {latestProgress.smmChange > 0 ? '+' : ''}{latestProgress.smmChange} lbs
                  </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  />
                  {weightLbs && (
                    <p className="text-slate-500">≈ {(parseFloat(weightLbs) * 0.453592).toFixed(1)} kg</p>
                  )}
                </div>
                  ...
                  return (
                    <TableRow key={scan.id}>
                      ...
                      <TableCell>
                        ...
                        {progress && (
                          <div className={progress.fatMassChange < 0 ? 'text-green-600' : progress.fatMassChange > 0 ? 'text-rose-600' : 'text-slate-600'}>
                            {progress.fatMassChange > 0 ? '+' : ''}{progress.fatMassChange} lbs
                          </div>
                        )}
                      </TableCell>
                      <TableCell>
                        ...
                        {progress && (
                          <div className={progress.leanMassChange > 0 ? 'text-green-600' : progress.leanMassChange < 0 ? 'text-rose-600' : 'text-slate-600'}>
                            {progress.leanMassChange > 0 ? '+' : ''}{progress.leanMassChange} lbs
                          </div>
                        )}
                      </TableCell>
                      <TableCell>
                        ...
                        {progress && (
                          <div className={progress.smmChange > 0 ? 'text-green-600' : progress.smmChange < 0 ? 'text-rose-600' : 'text-slate-600'}>
                            {progress.smmChange > 0 ? '+' : ''}{progress.smmChange} lbs
                          </div>
                        )}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Button onClick={() => handleDeleteScan(scan.id)} className="bg-rose-600 hover:bg-rose-700 text-white">
