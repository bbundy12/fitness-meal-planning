...
export function Dashboard({ currentUser, onNavigate }: DashboardProps) {
  // Mock data for current user - daily macro goals
  const [userData, setUserData] = useState({
    Me: { calories: 2250, protein: 169, carbs: 225, fat: 75 },
    Mom: { calories: 1800, protein: 113, carbs: 203, fat: 60 }
  });
  ...
  const data = userData[currentUser as keyof typeof userData];
  ...
  const handleSaveEdit = () => {
    setUserData({
      ...userData,
      [currentUser]: {
        calories: parseInt(editCalories) || data.calories,
        protein: parseInt(editProtein) || data.protein,
        carbs: parseInt(editCarbs) || data.carbs,
        fat: parseInt(editFat) || data.fat
      }
    });
    setIsEditing(false);
  };
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            {!isEditing ? (
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={handleSaveEdit} className="bg-rose-600 text-white hover:bg-rose-700">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => onNavigate('recipes')}>
          ...
          <Card className="cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => onNavigate('planner')}>
            <CardContent className="flex items-center gap-4 pt-6">
          ...
          <Card className="cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => onNavigate('shopping')}>
            <CardContent className="flex items-center gap-4 pt-6">
