...
export function RecipeBuilder() {
  ...
  const [recipes, setRecipes] = useState<Recipe[]>([
    ...
    },
    {
      ...
      servings: 1,
      instructions: "1. Bake salmon at 400°F for 12-15 minutes\n2. Roast sweet potato at 425°F for 25-30 minutes\n3. Season and serve",
      ingredients: [],
      totalCalories: 480,
      totalProtein: 38,
      totalCarbs: 45,
  ...
  const totals = calculateTotals();
  const perServing = {
    calories: Math.round(totals.calories / parseInt(servings || "1")),
    protein: Math.round(totals.protein / parseInt(servings || "1")),
    carbs: Math.round(totals.carbs / parseInt(servings || "1")),
    fat: Math.round(totals.fat / parseInt(servings || "1"))
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500">Create and manage your recipes with automatic macro calculations</p>
      </div>
      <Button onClick={() => setShowForm(!showForm)} className="bg-rose-600 hover:bg-rose-700">
        {showForm ? "Cancel" : <><Plus className="mr-2 h-4 w-4" />New Recipe</>}
      </Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-500">Type ingredient with quantity (e.g., "1 cup rice") and press Enter or click +</p>
            </div>
            ...
            <Button onClick={handleSaveRecipe} className="bg-rose-600 hover:bg-rose-700" disabled={!title.trim() || ingredients.length === 0}>
              Save Recipe
            </Button>
          </CardContent>
        </Card>
              ...
              <CardContent className="space-y-3">
                ...
                {recipe.instructions && (
                  <div>
                    ...
                    <p className="text-slate-900 whitespace-pre-line line-clamp-3">{recipe.instructions}</p>
                  </div>
                )}
